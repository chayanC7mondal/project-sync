...
async function testNotificationAPIs() {
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/hearings/mark-self-attendance`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.status === 404) {
        console.log('   ✅ API working (404 expected - no hearing session exists)');
...
function testNotificationConfig() {
  ...
  console.log('\n📋 Service Status:');
  if (config.smsEnabled && (process.env.TEXTBELT_KEY || process.env.SMS_GATEWAY_API_KEY)) {
