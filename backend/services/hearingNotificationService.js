...
export const sendPreHearingReminder = async (hearingSession) => {
  try {
    console.log(`[REMINDER] Sending pre-hearing reminders for hearing ${hearingSession._id}`);
    
    const caseDoc = await Case.findById(hearingSession.caseId).populate('investigatingOfficer');
    const attendanceRecords = await Attendance.find({ hearingSessionId: hearingSession._id }).populate('userId');
    ...
    if (caseDoc.investigatingOfficer) {
      ...
      await sendMultiChannelNotification(caseDoc.investigatingOfficer, notification);
    }
    ...
    console.log(`[REMINDER] Pre-hearing reminders sent successfully for ${caseDoc.caseId}`);
    return { success: true };
...
export const sendDayOfHearingReminder = async (hearingSession) => {
  try {
    console.log(`[REMINDER] Sending day-of-hearing reminders for hearing ${hearingSession._id}`);
...
export const sendPostHearingNotifications = async (hearingSession) => {
  try {
    console.log(`[POST-HEARING] Processing post-hearing notifications for hearing ${hearingSession._id}`);
    
    const caseDoc = await Case.findById(hearingSession.caseId).populate('investigatingOfficer');
    const attendanceRecords = await Attendance.find({ hearingSessionId: hearingSession._id }).populate('userId');
    
    const ioAttendance = attendanceRecords.find(a => a.userType === 'io');
    const witnessAttendances = attendanceRecords.filter(a => a.userType === 'witness');
    
    const ioPresent = ioAttendance && ioAttendance.status === 'present';
    const absentWitnesses = witnessAttendances.filter(a => a.status === 'absent');
    const presentWitnesses = witnessAttendances.filter(a => a.status === 'present');

    // Case 1: Witness absent, IO present
    if (!ioPresent && absentWitnesses.length > 0) {
      await handleBothAbsent(caseDoc, hearingSession, ioAttendance, absentWitnesses);
    }
    ...
    else if (ioPresent && witnessAttendances.every(a => a.status === 'present')) {
      await handleAllPresent(caseDoc, hearingSession, ioAttendance, presentWitnesses);
    }
 ...
 */
const handleBothAbsent = async (caseDoc, hearingSession, ioAttendance, absentWitnesses) => {
  // Notify absent IO
  ...
  for (const witnessAttendance of absentWitnesses) {
    if (witnessAttendance.userId) {
      ...
      await sendMultiChannelNotification(witnessAttendance.userId, notification);
    }
  }
 ...
 */
const handleWitnessesAbsent = async (caseDoc, hearingSession, absentWitnesses) => {
  for (const witnessAttendance of absentWitnesses) {
    if (witnessAttendance.userId) {
      const notification = {
        recipient: witnessAttendance.userId._id,
 ...
 */
const handleAllPresent = async (caseDoc, hearingSession, ioAttendance, presentWitnesses) => {
  // Confirm to IO
  if (ioAttendance && ioAttendance.userId) {
    const notification = {
      recipient: ioAttendance.userId._id,
 ...
 */
const notifySupervisors = async (caseDoc, hearingSession, alertType, details) => {
  try {
    ...
    for (const supervisor of supervisors) {
      ...
      switch (alertType) {
        case 'both_absent':
          message = `CRITICAL: Both Investigating Officer and ${details.witnessesAbsent} witness(es) were absent from court hearing for case ${caseDoc.caseId} on ${new Date(hearingSession.hearingDate).toDateString()}. Immediate investigation required.`;
          break;
