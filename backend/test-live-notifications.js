...
async function testNotificationSystemLive() {
  try {
    ...
    userNotifications.forEach((notif, index) => {
      console.log(
        `${index + 1}. ${notif.title} (${notif.priority}) - ${
    ...
    try {
      ...
      console.log(
        `   📧 Email: ${result.email?.success ? "SUCCESS" : "SKIPPED/FAILED"}`
      );
      console.log(
        `   📲 SMS: ${result.sms?.success ? "SUCCESS" : "SKIPPED/FAILED"}`
      );
    } catch (error) {
      console.log(`⚠️  Multi-channel test failed: ${error.message}`);
      console.log(
        "   (This is expected if SMS/Email services are not configured)"
      );
    }
    ...
    const stats = {
      ...
      highPriorityNotifications: await Notification.countDocuments({
        priority: "high",
      }),
      urgentNotifications: await Notification.countDocuments({
        priority: "urgent",
      }),
...
testNotificationSystemLive()
  .then((result) => {
    if (result.success) {
      ...
      console.log(
        "3. Create hearing sessions to trigger automated notifications"
      );
    } else {
