        ...
        for (const [carrier, gateway] of Object.entries(emailGateways)) {
          if (carrier === "airtel" || carrier === "jio" || carrier === "vi") {
            try {
              ...
              console.log(
                `[SMS] Email-to-SMS sent via ${carrier} gateway to ${phoneNumber}`
              );
              return { success: true, provider: `email-sms-${carrier}` };
            } catch (error) {
              console.warn(
                `[SMS] Email-to-SMS via ${carrier} failed:`,
                error.message
              );
            }
          }
...
export const sendEmail = async (
  ...
  try {
    ...
    console.log(
      `[EMAIL] Email sent successfully to ${to}, MessageId: ${result.messageId}`
    );
