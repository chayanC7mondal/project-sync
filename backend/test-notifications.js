...
const testSMS = async () => {
  ...
  const message =
    "Test SMS from Court Management System. Your hearing is scheduled for tomorrow at 10:00 AM.";
...
const testMultiChannel = async () => {
  ...
  const mockNotification = {
    ...
    message:
      "Your court hearing for case CR/001/2025 is scheduled today at 10:00 AM in District Court, Room 1. Please arrive 30 minutes early with all case documents.",
  ...
  try {
    ...
    if (results.sms) {
      console.log(
        `   ${results.sms.success ? "✅" : "❌"} SMS: ${
          results.sms.success
            ? "Success via " + results.sms.provider
            : results.sms.error
        }`
      );
    } else {
    ...
    if (results.email) {
      console.log(
        `   ${results.email.success ? "✅" : "❌"} Email: ${
          results.email.success
            ? "Success via " + results.email.provider
            : results.email.error
        }`
      );
    } else {
...
const checkEnvironment = () => {
  ...
  console.log(
    `   TEXTBELT_KEY: ${process.env.TEXTBELT_KEY ? "✅ Set" : "❌ Not set"}`
  );
  console.log(
    `   SMS_GATEWAY_API_KEY: ${
      process.env.SMS_GATEWAY_API_KEY ? "✅ Set" : "❌ Not set"
    }`
  );
  ...
  console.log(
    `   TEST_PHONE_NUMBER: ${process.env.TEST_PHONE_NUMBER || "Using default"}`
  );
};
